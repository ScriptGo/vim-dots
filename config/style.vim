...
" colorscehme
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
set t_Co=256             " 启用256色支持
set termguicolors        " 启用真色支持
set background=dark
color onedark
...
set statusline+=\ %f                            " 显示文件名
set statusline+=\ [%1*%M%*%n%R%H]               " 显示buffer 编号和状态
set statusline+=\ %y                            " 文件类型
set statusline+=%=                              " 向右对齐
set statusline+=\ %{&fileformat}\ [%{(&fenc==\"\"?&enc:&fenc).(&bomb?\",BOM\":\"\")}]
set statusline+=%=
set statusline+=\ [第%l行:第%c列]
set statusline+=\ [总计:%L行]
...
" 设置非可见字符的显示模式
set listchars=space:•,tab:»•,nbsp:+,extends:↷,precedes:↶,eol:⏎,trail:␣

" 设置非可见字符的显示颜色和格式
" 可见字符"eol"、"extends"、"precedes"是由NonText高亮组来控制显示颜色的
" 而"nbsp"、"tab"、"trail"则是由"SpecialKey"高亮组来定义的
...
if has("gui_running")
	...
	set guifont=Fira\ Code:h14,Sarasa\ Term\ SC\ Nerd:h14,Sarasa\ Term\ SC:h14
endif
